...
def get_system_prompt() -> str:
    return (
        ...
        "files with a unified API. Example:\n"
        "     from xlsb_reader import XlsxWorkbook, XlsbWorkbook, col_to_letter\n"
        "     wb = XlsxWorkbook(path)  # or XlsbWorkbook for .xlsb\n"
        "     for sheet_name, values in wb.iter_values():\n"
        "         # values is {(row, col): value} with 0-based indices\n"
        "         # only non-empty cells are included; merged cells return only the anchor\n"
        "     for sheet_name, formulas in wb.iter_formulas():\n"
        "         # formulas is {(row, col): formula_string} with 0-based indices\n"
        "   To build a DataFrame from a sheet's values dict, convert sparse → dense then "
        "drop all-None rows and columns: df.dropna(how='all').dropna(axis=1, how='all').\n"
